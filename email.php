...
<?php if ($request['request_type'] === 'access_request'): ?>
    <div class="access-details">
        <h4>Access Request Details</h4>
        <?php 
        $access_details = json_decode($request['access_details'], true);
        if ($access_details): 
        ?>
            <div class="access-grid">
                <div class="access-section">
                    <h5>Email Access</h5>
                    <ul>
                        <?php foreach ($access_details['email_access'] as $access): ?>
                            <li><?php echo ucfirst($access); ?></li>
                        <?php endforeach; ?>
                    </ul>
                </div>
                ...
                <div class="access-section">
                    <h5>USB Access</h5>
                    <ul>
                        <?php foreach ($access_details['usb_access'] as $access): ?>
                            <li><?php echo ucfirst($access); ?></li>
                        <?php endforeach; ?>
                    </ul>
                </div>
